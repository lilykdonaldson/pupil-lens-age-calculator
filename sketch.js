...
var spRatio = scotopicSum/photopicSum;
...
function setup() {
	...
	var A = exp(logA);
	var BermanModel =sqrt(A/PI)*2;
	input = createInput();
  input.position(20, 65);
	button = createButton('submit');
  button.position(input.x + input.width, 65);
  button.mousePressed(userSPD);

	ReaOuelletteModifiedDeGroot = 4.77-2.44*tan(0.3*log10(luminance))
	print("Rea/Ouellette: "+round(ReaOuelletteModifiedDeGroot,2));
	print("Berman Model: "+round(BermanModel,2));
}
